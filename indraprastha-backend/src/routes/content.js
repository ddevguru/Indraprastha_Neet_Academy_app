...
// Strips all Drive URLs/IDs from a chapter before sending to the client app.
// The app never sees the actual Drive link — it uses /chapter/:id/pdf-view instead.
function sanitizeChapterForClient(chapter) {
  const fileId = chapter.material_drive_file_id || extractDriveFileId(chapter.material_drive_link);
  const hasPdf =
    (chapter.material_type || '').toLowerCase().includes('pdf') && !!fileId;
  const { material_drive_link, material_drive_file_id, material_drive_folder_id, ...rest } = chapter;
  return { ...rest, has_pdf: hasPdf };
}
...
router.get('/books/:bookId/chapters', userAuth, async (req, res) => {
  ...
  if (chapters.rows.length > 0) {
    ...
    return res.json({ success: true, chapters: hydrated.map(sanitizeChapterForClient) });
  }
  ...
  const chapter = sanitizeChapterForClient({
...
router.get('/chapters/:chapterId', userAuth, async (req, res) => {
  ...
  const extracted = await ensureChapterExtracted(chapter.rows[0]);
  res.json({ success: true, chapter: sanitizeChapterForClient(extracted) });
});

// Serves a secured HTML page that embeds the Drive PDF viewer.
// The Drive file ID never leaves the server — the app only knows the chapter ID.
router.get('/chapter/:chapterId/pdf-view', userAuth, async (req, res) => {
  const result = await pool.query(
  ...
  const err = (msg) =>
    `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      ...
      p{color:#555;font-size:16px}</style></head><body><p>${msg}</p></body></html>`;

  if (result.rows.length === 0) return res.status(403).send(err('Access denied.'));
  const ch = result.rows[0];
  if (!(ch.material_type || '').toLowerCase().includes('pdf'))
    return res.status(404).send(err('No PDF for this chapter.'));

  const fileId = ch.material_drive_file_id || extractDriveFileId(ch.material_drive_link);
  if (!fileId) return res.status(404).send(err('PDF not available yet.'));
...
#btm-shield{position:fixed;bottom:0;left:0;width:100%;height:52px;z-index:9999;pointer-events:all}
</style>
</head>
<body>
<iframe src="${src}" allowfullscreen="false" referrerpolicy="no-referrer"></iframe>
<div id="btm-shield"></div>
<script>
document.addEventListener('contextmenu',function(e){e.preventDefault();return false},true);
document.addEventListener('keydown',function(e){
  if((e.ctrlKey||e.metaKey)&&['s','p','c','a','u','j'].includes(e.key.toLowerCase()))
    e.preventDefault();
},true);
</script>
</body>
...
router.get('/mcq-of-day', userAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, question, option_a, option_b, option_c, option_d, correct_option,
              explanation, question_image_link, question_image_drive_file_id,
              subject, topic, issued_at
       FROM mcq_of_day
       WHERE (batch_id = $1 OR batch_id IS NULL)
         AND issued_at <= NOW()
    ...
    console.error('[MCQ_OF_DAY_ERROR]', { message: e?.message, stack: e?.stack });
